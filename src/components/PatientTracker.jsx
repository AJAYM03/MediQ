...
export default function PatientTracker() {
  ...
  // 2. Fetch Doctor Profile (For OP Times and Room)
  useEffect(() => {
  ...
  // 3. Fetch Live Queue Engine (For Rolling Average and Current Serving)
  useEffect(() => {
    if (!ticket?.doctor_id) return;
  ...
  const getDynamicETA = () => {
    if (!ticket || !docProfile?.op_schedule || !queueEngine) return { time: "--:--", ahead: 0, liveAvg: 5 };
  ...
  if (!ticket || !queueEngine) return <div className="p-10 text-center font-bold">Verifying token...</div>;
        ...
        <div className="p-8 text-center space-y-6">
          ...
          <div className="bg-blue-50 rounded-2xl p-6 border border-blue-100 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center justify-center gap-2 mb-4">
              ...
              <p className="text-blue-800 font-bold text-sm">Estimated Consultation Time</p>
              <span className="text-3xl font-black text-blue-900">
                {ticket.status === 'completed' ? "--:--" : etaData.time}
              </span>
            </div>
